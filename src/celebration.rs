...
impl CelebrationParticle {
    ...
    fn new_text_particle(x: f64, y: f64, target_x: f64, target_y: f64, symbol: char, color: usize) -> Self {
        use rand::Rng;
        let mut rng = rand::thread_rng();
    ...
    fn update(&mut self, dt: f64) -> bool {
        if self.is_text {
            ...
            let dist_to_target = ((self.target_x - self.x).powi(2) + (self.target_y - self.y).powi(2)).sqrt();
...
impl CelebrationAnimation {
    ...
    pub fn start(&mut self, width: u16, height: u16) {
        ...
        let words = ["PERFECT!", "AMAZING!", "EXCELLENT!", "FLAWLESS!", "SUPERB!", "BRILLIANT!"];
    ...
    fn create_text_particles(&mut self, text: &str, center_x: f64, center_y: f64, rng: &mut rand::rngs::ThreadRng) {
        use rand::Rng;
...
mod tests {
    ...
    fn test_text_particle_movement() {
        ...
        let distance = ((text_particle.target_x - text_particle.x).powi(2) + 
                       (text_particle.target_y - text_particle.y).powi(2)).sqrt();
    ...
    fn test_celebration_particle_movement() {
        ...
        let initial_positions: Vec<(f64, f64)> = celebration.particles
            .iter()
            .map(|p| (p.x, p.y))
            .collect();
        ...
        let moved_count = celebration.particles
            .iter()
        ...
        println!("✅ {} out of {} particles moved", moved_count, celebration.particles.len());
    }
    ...
    fn test_particles_removed_when_off_screen() {
        ...
        celebration.particles.push(CelebrationParticle::new(100.0, 100.0)); // Way off screen
        ...
        for particle in &celebration.particles {
            if !particle.is_text {
                ...
                assert!(!off_screen, "Particle at ({}, {}) should have been removed", particle.x, particle.y);
            }
        }
